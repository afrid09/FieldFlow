...
app.use((req, res, next) => {
  ...
  (req as Req & { id?: string }).id = requestId;
  next();
});
...
// Decode JWT without enforcing roles (used by auth middleware).
...
// Best-effort event emission to gateway for realtime UI invalidation.
